...
static volatile uint8_t set_idle_buffer = 0;
static volatile uint8_t set_protocol_buffer = 1;
...
inline void configure_ep(int epnum){
	const uint8_t* ep = ((const uint8_t *) ep_list) + epnum*sizeof(struct ep_list_struct); // ==&ep_list[0+offset]
    UENUM = epnum;
    UECONX = 1;
    ...
    if (UEINTX & (1<<RXSTPI)) {
        ...
        if (REQUEST_TYPE(bmRequestType) == STANDARD_REQUEST) {
            if (bRequest == GET_DESCRIPTOR) {
                ...
                const uint8_t* desc = (const uint8_t *) desc_list;
                uint16_t desc_val;
                uint16_t desc_ind;
        ...
        if (REQUEST_TYPE(bmRequestType) == CLASS_REQUEST) { // HID request
            if (REQUEST_DIRECTION(bmRequestType) == REQUEST_IN) {
                ...
                if (bRequest == HID_GET_IDLE) {
                    usb_wait_in_ready();
                    UEDATX = set_idle_buffer;
                    usb_send_in();
                    return;
                }
                if (bRequest == HID_GET_PROTOCOL) {
                    usb_wait_in_ready();
                    UEDATX = set_protocol_buffer;
                    usb_send_in();
                    return;
                }
            }
            if (REQUEST_DIRECTION(bmRequestType) == REQUEST_OUT) {
                ...
                if (bRequest == HID_SET_IDLE) {
                    set_idle_buffer = MSB(wValue);
                    usb_send_in();
                    return;
                }
                if (bRequest == HID_SET_PROTOCOL) {
                    set_protocol_buffer = wValue;
                    usb_send_in();
                    return;
                }
            }
